...
- GitHub Actions `CI` runs on the PR, not after merge to `main`. Merge with squash+fast-forward; `main` only deploys. Keep that one check required in branch protection so untested code cannot merge.
