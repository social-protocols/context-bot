defmodule ContextBot.StandardSite.ReaderReady do
  ...
  `reader_checked_at` inside the 60s negative TTL stays waiting without calling
  ...
  @doc "Negative-cache window for a fresh `reader_checked_at` miss."
  ...
  `ensure/2` uses this so follower-card waits skip
  `app.standard-reader.getDocument` for one window.
  """
