defmodule Mix.Tasks.ContextBot.ReenqueueTest.Service do
  @moduledoc false

  def reenqueue(invocation_id, options) do
    config = Application.fetch_env!(:context_bot, __MODULE__)
    send(config[:test_pid], {:reenqueue, invocation_id, options})
    config[:result]
  end
end

defmodule Mix.Tasks.ContextBot.ReenqueueTest.Runtime do
  @moduledoc false
...
defmodule Mix.Tasks.ContextBot.ReenqueueTest do
  use ExUnit.Case, async: false

  alias ContextBot.Workflow.Invocation
  alias Mix.Tasks.ContextBot.Reenqueue, as: ReenqueueTask
  alias Mix.Tasks.ContextBot.ReenqueueTest.{Runtime, Service}

  setup do
    original_shell = Mix.shell()
    original_task = Application.get_env(:context_bot, ReenqueueTask, :missing)
    ...
    Application.put_env(:context_bot, ReenqueueTask,
      reenqueuer: Service,
      runtime: Runtime,
      now: fn -> ~U[2026-09-03 00:30:00.000000Z] end
    )
    ...
    on_exit(fn ->
      ...
      restore_env(ReenqueueTask, original_task)
  ...
  test "loads configuration before starting the application" do
    assert ReenqueueTask.__info__(:attributes)[:requirements] == ["app.config"]
  end
  ...
  test "requires exactly one positive integer invocation ID before startup" do
    for arguments <- [[], ["1", "2"], ["0"], ["-1"], ["nope"], ["1suffix"]] do
      ...
      refute_received {:reenqueue, _, _}
    end
  end

  test "starts the application and prints only the reenqueued identity" do
    assert :ok = run(["42"])

    assert_received :application_started
    assert_received {:reenqueue, 42, [now: ~U[2026-09-03 00:30:00.000000Z]]}
    assert shell_output() == "status=reenqueued\ninvocation_id=42\n"
  end
  ...
  test "just reenqueue delegates to the Mix task" do
    {recipe, 0} =
      System.cmd("just", ["--dry-run", "reenqueue", "42"], stderr_to_stdout: true)

    assert recipe == "mix context_bot.reenqueue '42'\n"
  end

  defp run(arguments) do
    Mix.Task.reenable("context_bot.reenqueue")
    ReenqueueTask.run(arguments)
  end
