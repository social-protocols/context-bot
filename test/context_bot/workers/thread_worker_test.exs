...
defmodule ContextBot.Workers.ThreadWorkerTest do
  ...
  test "captures a video thread and proceeds to research with video flag" do
    invocation = dry_run_invocation()
    ...
    assert persisted.stage == :thread_ready
    assert persisted.status == :thread_ready
    assert persisted.contains_video == true
  ...
  test "captures a public video thread and proceeds to research with video flag" do
    invocation = invocation()
    response = fixture("thread_video.json")

    configure_fake({:ok, 200, %{}, response}, now: fn -> @now end)
    ...
    # Video threads now go through normal research path
  ...
  test "rolls back a public thread snapshot when research job enqueue fails" do
    invocation = invocation()
    response = fixture("thread_video.json")
    ...
    invalid_job_builder = fn transitioned_invocation ->
      ...
      |> Ecto.Changeset.add_error(:args, "forced research job failure")
    end

    configure_fake({:ok, 200, %{}, response},
      now: fn -> @now end,
      research_job_builder: invalid_job_builder
    )
