...
#     E.g.: docker.io/hexpm/elixir:1.20.2-erlang-28.5.0.3-debian-trixie-20260713-slim
...
RUN apt-get update \
  && apt-get install -y --no-install-recommends libstdc++6 openssl libncurses6 locales ca-certificates libsqlite3-0 sqlite3 gosu \
  && rm -rf /var/lib/apt/lists/*
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/context_bot ./
