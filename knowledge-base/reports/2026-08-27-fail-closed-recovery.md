...
| Parse / tools | Unknown `server_tool_use` → `:unexpected_tool_use`. Native `web_search`/`web_fetch` plus code-execution subtools are allowlisted. Non-zero `return_code` / tool-result error → `:code_execution_failed`. In-band `max_uses_exceeded` is a completed turn, not a hard-fail. Truncation/refusal/max_tokens fail closed. | Failed `provider_response`; envelope kept. | Automatic recovery does **not** reopen parser hard-fails. Operator reprocess may replay, except `code_execution_failed`, which starts a new paid attempt. |
