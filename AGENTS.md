...
| `just recover [id]` / `fly-recover` | Run the same `Recovery.recover_orphans/1` path as boot; optional id uses `recover_invocation/2`. May publish a Bluesky reply. |
...
This is one Phoenix API application, not an umbrella. `ContextBot.Application` starts Repo and Finch always, and starts Oban, `ContextBot.ATProto.Session`, and `ContextBot.Mentions.Poller` only when the validated settings enable the bot. `GET /health` returns bounded operational aggregates and never stored content or credentials. Public `GET /invocations` is a GET-only HTML log of operational metadata (counts, spend, tokens, status, actor handle, Bluesky and Standard.site links, short error reasons). That HTML page does not show post bodies, prompts, envelopes, or secrets. Public JSON dumps (`GET /invocations.json`, `GET /invocations/:id.json`, `GET /api_budget_entries.json`, `GET /anthropic_response_envelopes.json`, and their per-id routes) intentionally expose every product-table column, including Base64 `raw_body` and claim tokens, for transparency. Public `GET /r/:id` is a legacy 301 to the Standard Reader URL from `standard_site_document_uri` (invocation id or document rkey); it does not serve writeups. New Bluesky `(full response)` and follower-card links still use `Document.reader_url_from_uri/1`. None of these public surfaces have reprocess, reenqueue, recover, or other mutation endpoints. General failed-invocation recovery is `ContextBot.Workflow.Recovery.recover_orphans/1` at boot via `StartupRecovery`, or the same function through `just fly-recover` / `mix context_bot.recover`. Envelope replay stays on `just fly-reprocess` over Fly SSH. A fresh two-phase research run of the same invocation id stays on `just fly-reenqueue`.
...
- admit every exact direct mention and classify the actor into a rate-limit tier: operator allowlist (skip actor hourly/daily windows), confirmed Skywatch `bluesky-elder` or bidirectionally verified `bsky.team` (`ACTOR_DAILY_LIMIT`, default 5), or public (`ACTOR_DAILY_LIMIT_PUBLIC`, default 1); a labeler or identity outage degrades to public instead of rejecting; `ACTOR_HOURLY_LIMIT` remains a burst cap for non-operator actors; mentions whose parent post is by the bot do not consume actor hourly/daily slots; `THREAD_DAILY_LIMIT` (default 3) caps research admissions per thread root per rolling day for everyone, including operators and follow-ups; global hourly/daily and `max_pending` still apply to everyone; actor-rate, thread-rate, and budget limits post a short Bluesky notice linking `https://getcontext.bot/` (skip only a duplicate notice already posted for that actor in the window; do not silent-complete because the parent is a bot reply);
...
- after the part-1 Bluesky reply is published, best-effort update the Standard.site document `bskyPostRef` to a `com.atproto.repo.strongRef` of that reply (`reply_uri` + `reply_cid`) so off-platform comments attach to our reply, not the invoking mention, follower post, or later parts; log failures and do not block publication;
...
- default the structure-phase model to `ANTHROPIC_MODEL_ID` when `ANTHROPIC_STRUCTURE_MODEL_ID` is unset; send `thinking: {type: "disabled"}` on `Request.structure/1` and `structure_repair/1` (Sonnet 5 turns adaptive thinking on when the field is omitted; do not use a `budget_tokens` tiny-think fallback — Sonnet 5 400s that). Research stays adaptive. Leave title rewrite on `ANTHROPIC_TITLE_MODEL_ID` and omit thinking there;
- when structured `disposition=reply` has a blank `title` after trim but nonempty in-cap `compact_reply` and `full_response`, rewrite the title with a cheap `ANTHROPIC_TITLE_MODEL_ID` (default Haiku) call that reuses the leftover `:repair` reservation and `ANTHROPIC_LENGTH_REPAIR_MAX_TOKENS`; do not rewrite `compact_reply`; leftover `invalid_structured_output` rows stay failed until operator reprocess;
- publish a Bluesky reply only when the invoking mention has an obvious question or request aimed at this bot; claim-only counterarguments (inv 35 diogeneslamp shape) are `no_reply` with an empty `CONTEXT_BOT_DRAFT`; word `compact_reply` as a reply to that invoking post so the Bluesky body is not a floating “Both claims check out…”; do not add more enumerate-claims language; do not recover or rewrite inv 35 or 37 from this PR;
- research writes a parseable `CONTEXT_BOT_DRAFT` title and compact_reply (target 275 graphemes, hard cap 300 graphemes / 3,000 UTF-8 bytes) before the thorough writeup; structure prefers those drafts and may shorten or lightly rewrite them to meet the cap; code parses the drafts and includes the full draft text (even when over cap), measured grapheme/byte counts, over-cap delta, and an optional programmatic truncate ≤300 graphemes / 3,000 bytes as a suggested seed in the structure user turn using the same counters as publication; do not omit over-cap draft text from the banner and do not strip `CONTEXT_BOT_DRAFT` from the stored research writeup (structure and recover still parse it); strip the draft block from published Standard Reader / `site.standard.document` writeup bodies; a parse miss supplies no draft banner and structure writes a short compact from the writeup; empty compact, structure `max_tokens`, or overlong compact that cannot be packed into ≤2 Bluesky posts (including the full-response link) or locally shortened, starts at most one compact-repair call; prefer local pack-first split, then a hard truncate whose published body still fits in ≤2 posts including the link, with a continuation ellipsis immediately before ` (full response)`; truncation is Bluesky-only — do not replace or shorten the compact stored in the writeup / Reader document (keep the research-draft compact, or the pre-truncate compact if there is no draft); if structure or repair hard-fails and a research draft compact is under cap or truncatable / packable into ≤2 posts, publish that draft instead of failing closed; do not send schema `minLength`/`maxLength`; do not raise `ANTHROPIC_STRUCTURE_MAX_TOKENS` or change research max tokens;
- recover durable work oldest-first with bounded scans; automatic `recover_failed` reopens interruptions, locally retryable 2xx envelopes, and a stored research writeup whose last structure or provider call is not a replayable 2xx (live `Request.structure/1`, no 4xx replay, no second research payment); it does not reopen published rows or deterministic parser hard-fails (`code_execution_failed`, `invalid_structured_output`, `unexpected_tool_use`, and other parser reasons that will not change on replay); operator `just recover <id>` / `just fly-recover <id>` passes `operator?: true` and retries structure-from-writeup for `max_tokens`, `invalid_structured_output`, `empty_compact`, `overlong_compact`, and `invalid_repair` without clearing research; Runner then POSTs a new `:structure` call instead of replaying a latest 2xx structure or repair hard-fail (`max_tokens`, `invalid_structured_output`, `empty_compact`, `overlong_compact`); successful 2xx and non-structure-from-writeup envelopes still replay; operator reprocess of `code_execution_failed` starts a new paid attempt; after merge/deploy, inv 30 is `just fly-recover 30` (do not reenqueue; do not run it from this PR); leave inv 35 and 37 failed (do not fly-recover or reenqueue them from this PR);
