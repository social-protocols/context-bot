...
This is one Phoenix API application, not an umbrella. `ContextBot.Application` starts Repo and Finch always, and starts Oban, `ContextBot.ATProto.Session`, and `ContextBot.Mentions.Poller` only when the validated settings enable the bot. `GET /health` returns bounded operational aggregates and never stored content or credentials. Public `GET /invocations` is a GET-only HTML log of operational metadata (counts, spend, tokens, status, actor handle, Bluesky and Standard.site links, short error reasons). It does not show post bodies, prompts, envelopes, or secrets, and it has no reprocess, reenqueue, recover, or other mutation endpoints. Public `GET /r/:id` (and `/r/{document_rkey}`) serves a getcontext.bot HTML mirror of a published full-response writeup rebuilt from sqlite via `Document.format_markdown/1`. New Bluesky `(full response)` facets point at that mirror. The page 302s to Standard Reader once `app.standard-reader.getDocument` reports a matching `uri` and `hasRenderableBody`; a miss or ambiguous probe stays on the mirror. PDS / Standard.site publication is unchanged. General failed-invocation recovery is `ContextBot.Workflow.Recovery.recover_orphans/1` at boot via `StartupRecovery`, or the same function through `just fly-recover` / `mix context_bot.recover`. Envelope replay stays on `just fly-reprocess` over Fly SSH. A fresh two-phase research run of the same invocation id stays on `just fly-reenqueue`.
