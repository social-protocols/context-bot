...
- when the structure call hits `stop_reason=max_tokens` (truncated/invalid JSON) and a research writeup is stored, make at most one compact-repair `Request.structure_repair/1` call on the leftover `:repair` reservation with `ANTHROPIC_LENGTH_REPAIR_MAX_TOKENS` so a writeup-length `compact_reply` cannot fit; fail closed if that repair also fails; do not raise `ANTHROPIC_STRUCTURE_MAX_TOKENS` or change research max tokens; research-phase `max_tokens` stays terminal;
- recover durable work oldest-first with bounded scans; automatic `recover_failed` reopens interruptions, locally retryable 2xx envelopes, and a stored research writeup whose last structure or provider call is not a replayable 2xx (live `Request.structure/1`, no 4xx replay, no second research payment); it does not reopen published rows or deterministic parser hard-fails (`code_execution_failed`, `invalid_structured_output`, `unexpected_tool_use`, and other parser reasons that will not change on replay); operator reprocess of `code_execution_failed` starts a new paid attempt; `just fly-recover` runs this same recovery;
