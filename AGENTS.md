...
- default the structure-phase model to `ANTHROPIC_MODEL_ID` when `ANTHROPIC_STRUCTURE_MODEL_ID` is unset; keep `Request.structure/1` without thinking or `output_config.effort`; leave title rewrite on `ANTHROPIC_TITLE_MODEL_ID`;
